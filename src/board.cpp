  ...
  if (current_cell->down_wall) {
    ...
    if (current_cell->right_wall) { // !down right
      ...
      if (row == HEIGHT - 1 || column == WIDTH - 1 ||
          current_cell->empty_bottom_right)
...
bool Board::ongoing_line_clearing() {
  ...
  if (dropping_column < 0) {
    ...
    while (current_row >= 0) {
      ...
      if (source_row < current_row) {
        bool source_occupied =
            source_row < 0 ? false : dropping_column_new_states[source_row];

        if (source_occupied) {
          dropping_column_new_states[current_row] = true;
          if (source_row >= 0)
            dropping_column_new_states[source_row] = false;
        } else {
    ...
    for (u8 changes = 0; dropping_row >= 0 && changes < LINE_CLEARING_BUDGET;
         dropping_row--) {
      if (dropping_column_new_states[dropping_row] !=
          occupied(dropping_row, dropping_column)) {
        changes++;
        if (dropping_column_new_states[dropping_row]) {
